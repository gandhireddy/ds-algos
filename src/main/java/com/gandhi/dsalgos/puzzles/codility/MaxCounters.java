...
public class MaxCounters {
	...
	public int[] solution(int N, int[] A) {
		// Initializes the array by default values: '0' for int.
		int[] counters = new int[N];
		int maxCount = 0;

		boolean setMaxCountDetected = false;
		for (int i = 0; i < A.length; i++) {
			if (A[i] >= 1 && A[i] <= N) {
				if (setMaxCountDetected) {
					Arrays.fill(counters, maxCount);
					setMaxCountDetected = false;
				}
				counters[A[i] - 1]++;
